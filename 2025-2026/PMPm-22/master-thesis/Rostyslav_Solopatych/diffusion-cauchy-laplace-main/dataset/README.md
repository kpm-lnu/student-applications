...
```bash
python dataset/generate_harmonic_dataset.py --output ./harmonic_field_100k.pt \
 --num_samples 125000 \
 --val_split 0.2 \
 --pixel_res 64 \
 --N 8 \
...
1. **Sparse Solutions** (up to N/2 active terms): Creates focused, localized boundary conditions with variable magnitude scaling (10⁻² to 10²)
2. **Power-Law Decay** (α ∈ [1, 2]): Realistic frequency content with higher modes decaying
...
- ✅ Finite values check
- ✅ Interior coverage > 90%

Invalid samples are automatically replaced with re-generated samples.
