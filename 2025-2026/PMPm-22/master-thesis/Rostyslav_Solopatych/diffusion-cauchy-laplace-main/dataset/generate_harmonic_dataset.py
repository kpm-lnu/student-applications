...
def generate_diverse_coefficients(N, diversity_mode='mixed'):
    ...
    if diversity_mode == 'sparse':
        ...
        a = np.zeros(N)
        num_active = np.random.randint(1, N//2 + 1)  # N div 2 active terms
        exp_mean_power = np.random.randint(-2, 2)  # Mean power between 10^-2 and 10^2
        exp_mean_base = np.random.uniform(0, 1)
        exp_mean = exp_mean_base * (10 ** exp_mean_power)
        active_indices = np.random.choice(N, num_active, replace=False)
        # Use exponential distribution for magnitudes to get variety
        magnitudes = np.random.exponential(exp_mean, num_active)
        ...
        decay_rate = np.random.uniform(0.5, 2)  # Decay factor
        n_indices = np.arange(N)
        alternating = (-1)**n_indices
        decay = decay_rate**(-n_indices)
        base_amplitude = np.random.uniform(0.1, 1.0)
        ...
        a = np.zeros(N)
        band_width = np.random.randint(1, N // 2 + 1)
        num_bands = N // band_width + 1

        for start in range(0, num_bands, band_width):
            end = min(N, start + band_width)
            # Fill band with correlated coefficients
            band_amplitude = np.random.uniform(0.5, 2.0)
            band_coeffs = np.random.randn(end - start) * band_amplitude
            a[start:end] += band_coeffs
