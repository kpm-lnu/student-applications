#!/usr/bin/env python3
"""
Plot L∞ (supremum norm) metrics for physics components from epoch_losses.json file.

This script reads the epoch losses JSON file and generates plots showing
the L∞ metrics for Laplace, Dirichlet, and Neumann components across all epochs.
"""
...
def plot_linf_metrics(json_file, batch_size, learning_rate, pixel_res, output_dir=None):
    """
    Plot L∞ metrics for physics components from epoch_losses.json file.
    ...
    suptitle = f'L∞ Metrics (Worst-Case Errors): BS: {batch_size}; LR: {learning_rate:.1e}; Pixel resolution: {pixel_res}x{pixel_res}'
    fig.suptitle(suptitle, fontsize=14, fontweight='bold')
    
    # Plot Laplace L∞
    axes[0].plot(epochs, laplace_linf, label='Laplace L∞', marker='o', markersize=3, linewidth=1.5, color='#1f77b4')
    axes[0].set_xlabel('Epoch', fontsize=11)
    axes[0].set_ylabel('L∞ (max absolute error)', fontsize=11)
    axes[0].set_title('Laplace Equation (∇²u = 0)', fontsize=12)
    axes[0].legend(fontsize=9)
    axes[0].grid(True, alpha=0.3)
    
    # Plot Dirichlet L∞
    axes[1].plot(epochs, dirichlet_linf, label='Dirichlet L∞', marker='s', markersize=3, linewidth=1.5, color='#ff7f0e')
    axes[1].set_xlabel('Epoch', fontsize=11)
    axes[1].set_ylabel('L∞ (max absolute error)', fontsize=11)
    axes[1].set_title('Dirichlet BC (u = g)', fontsize=12)
    axes[1].legend(fontsize=9)
    axes[1].grid(True, alpha=0.3)
    
    # Plot Neumann L∞
    axes[2].plot(epochs, neumann_linf, label='Neumann L∞', marker='^', markersize=3, linewidth=1.5, color='#2ca02c')
    axes[2].set_xlabel('Epoch', fontsize=11)
    axes[2].set_ylabel('L∞ (max absolute error)', fontsize=11)
    ...
    output_file = os.path.join(output_dir, 'linf_metrics.png')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Plot L∞ metrics from epoch_losses.json')
