...
class Checkbox:
    ...
    def draw(self, surface):
        ...
        if self.checked:
            pygame.draw.line(surface, (0, 150, 0), (self.rect.x + 2, self.rect.y + self.rect.h/2), (self.rect.centerx, self.rect.bottom - 2), 3)
            pygame.draw.line(surface, (0, 150, 0), (self.rect.centerx, self.rect.bottom - 2), (self.rect.right - 2, self.rect.top + 2), 3)
...
class App:
    ...
    def start_simulation(self):
        ...
        if self.opt_checkbox.checked:
            self.app_state = "OPTIMIZING"
            self.optimization_done = False
            self.draw() 
            pygame.display.flip()
            ...
            self.sim_results, self.t_eval = simulation_engine.run_simulation(
                start_day, sim_days, self.initial_conditions, self.sim_data, simulation_engine.DEFAULT_PARAMS 
            )
            self._setup_animation_start(start_day)

    def _setup_animation_start(self, start_day):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: 
                    ...
                    if clicked_on_ui:
                        ...
                        if self.app_state == "SETUP" and self.opt_checkbox.rect.collidepoint(ui_mouse_pos):
                            self.opt_checkbox.checked = not self.opt_checkbox.checked
                        
                        elif self.simulation_finished and self.graph_button_rect and self.graph_button_rect.collidepoint(ui_mouse_pos):
                            self.show_graph()
                        else:
    ...
    def draw_ui_panel(self):
        ...
        if self.app_state == "SETUP":
            ...
            self.opt_checkbox.draw(self.ui_surface)
        ...
        elif self.app_state == "OPTIMIZING":
            text = self.font_large.render("НАВЧАННЯ СУРОГАТУ...", True, (0, 0, 200))
            self.ui_surface.blit(text, (px, py_status))
            text2 = self.font_medium.render("(Дивіться логи у консолі)", True, (100, 100, 100))
            self.ui_surface.blit(text2, (px, py_status + 30))
            
        elif self.app_state == "SIMULATING":
            text = self.font_large.render("РОЗРАХУНОК СИСТЕМИ...", True, (200, 0, 0))
            self.ui_surface.blit(text, (px, py_status))
