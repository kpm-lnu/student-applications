...
plt.xlabel('Alpha')
plt.ylabel('Max Prey')
plt.title('RBF-інтерполяція для моделі хижак-жертва')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

# Функція для побудови метамоделі на нових межах
alpha_dense = np.linspace(0.3, 1.5, 500).reshape(-1, 1)
y_dense_pred = rbf_model(alpha_dense)

# Візуалізація з метамоделлю
plt.figure(figsize=(8, 5))
plt.scatter(alpha_train, y_train, color='red', label='Навчальні точки')
plt.scatter(alpha_test, y_test_pred, color='blue', label='Прогнозовані тестові точки')
plt.plot(alpha_dense, y_dense_pred, color='green', label='Сурогатна модель', linewidth=2)
plt.xlabel('Alpha')
plt.ylabel('Max Prey')
plt.title('RBF-інтерполяція для моделі хижак-жертва')
plt.legend()
